...
const AssignBatchSupport = (props) => {
        ...
        <tr>
          ...
          <td className="px-6 py-4 whitespace-nowrap text-center">
            {props.items.assign_batch_details != null ? <>{`${props.items.assign_batch_details?.BatchID.BatchName} (${props.items.assign_batch_details?.BatchID?.BatchTags})`} <br /> <span className='text-sm'>{props.items.assign_batch_details?.BatchID?.BatchTime.substring(0, 5)} - {props.items.assign_batch_details?.BatchID?.BatchEndTime.substring(0, 5)}</span>
              ...
              <button
                ...
                onClick={() => {
                  ...
                  axios.delete(`${API_BASE_URL}/batchstudent/${convertedId}/`, {
                    ...
                    props.batchAssignStuDetailsFunc();
                  }).catch((err) => {
                    console.log(err);
                  }).finally(()=>{
              ...
              >
                {removeButton ? <>&nbsp; &nbsp; <CircularProgress size={19} color='inherit'/> &nbsp; &nbsp;</>: "Remove"}
            ...
            >
              ...
              {props.batches?.map((element, index) => {
                return <option value={element.BatchID} key={index}>{`${element.BatchName} (${element.BatchTags})`} {element.BatchTime.substring(0, 5)} - {element.BatchEndTime.substring(0, 5)}  {element.BatchStartDate ? `(${format(new Date(element?.BatchStartDate), "dd MMM yyyy")})` : "----"}</option>
              })}
            </select> &nbsp;
              <br />
              <button
                ...
                onClick={() => {
                  ...
                  if (batchId != null && batchId != undefined) {
                    axios.post(`${API_BASE_URL}/batchstudent/`, {
                      BatchID: batchId,
                      CustomerID: props?.items?.StudentID.CustomerID,
                      ...
                      toast.success('Batch Assigned Sucessfully', {
                ...
                }}>{assignButton ? <>&nbsp; &nbsp; <CircularProgress size={19} color='inherit'/> &nbsp; &nbsp;</>: "Assign"}</button></> : ""}
            ...
            <div className='text-center mt-4 p-4 rounded-xl bg-gray-200'>
              ...
              >
                ...
                {props.batches?.map((element, index) => {
                  console.log()
                  console.log(element)
                  return <option value={element.BatchID} key={index}> {`${element.BatchName} (${element.BatchTags})`} {element.BatchTime.substring(0, 5)} - {element.BatchEndTime.substring(0, 5)}  {element.BatchStartDate ? element?.BatchStartDate.substring(0, 10) : "----"}</option>
                })}
              </select> &nbsp;
                <br />
                <button
...
export default AssignBatchSupport;