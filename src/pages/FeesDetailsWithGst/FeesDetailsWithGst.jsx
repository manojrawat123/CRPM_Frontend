...
const FeesDetails = () => {
  const { GetFeesAll, allFeesObj, getFilterFeesFunc, filteredallFeesObj } = useContext(DataContext);
  ...
  const handleSelect = (date) => {
    ...
    getFilterFeesFunc(date.selection.startDate, date.selection.endDate)
  };
  ...
  return (
    <>
            ...
            {isFeesDate ? (
              <ExcelDownloadButton
                data={allFeesObj?.map((element, index) => ({
                  ...
                  GST: parseInt(element.fee_received, 10) - (parseInt(element.fee_received, 10) / 1.18).toFixed(2),
                  "Fee without GST": (parseInt(element.fee_received, 10) / 1.18).toFixed(2),
      ...
      <div className="m-8">
        ...
        {isFeesDate ? allFeesObj ? allFeesObj.length == 0 ? <NoDataPage status={"Fees"} /> : null : null : null}
        {!isFeesDate ? filteredallFeesObj ? filteredallFeesObj.length == 0 ? <NoDataPage status={"Fees"} /> : null : null : null}
