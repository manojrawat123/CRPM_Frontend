  ...
  return (
    <>
      ...
      <div className="my-4 text-center font-bold text-2xl">
        <h1 className="text-green-500 underline">Email Sender Form</h1>
      </div>
      <div className="green-theme md:w-[60%] w-[90%] mx-8 md:mx-auto  mt-8 border-2 border-solid border-gray-500 shadow-xl rounded-xl p-4">
        <Formik
          ...
          onSubmit={(values, { resetForm }) => {
            ...
            const myLeadScource = values?.leadscource?.map((element, index) => {
              return element.value;
            });
            console.log(myLeadScource);
            console.log(filteredData);
            const filteredDataLeadStatus = filteredData.filter((lead) => {
              return myleadStatus.includes(lead.LeadStatus);
            });
            const filteredDataLeadStatusCourse = filteredDataLeadStatus.filter(
              (lead) =>
                lead.LeadServiceInterested.some((service) =>
                  myleadCourse.includes(service.ServiceName)
                )
            );
            console.log(filteredDataLeadStatusCourse);
            const filterdeDataLeadStatusCourseSource =
              filteredDataLeadStatusCourse.filter((lead) => {
                return myLeadScource.includes(lead.LeadSource);
              });
            const endData = filterdeDataLeadStatusCourseSource?.filter(
              (lead) => {
                return !list.includes(lead?.LeadPhone);
              }
            );
            const excludedList = filterdeDataLeadStatusCourseSource?.filter(
              (lead) => {
                return list.includes(lead?.LeadPhone);
              }
            );
            ...
            <Form>
                  ...
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      onChange={(selectedOptions) =>
                        setFieldValue("leadStatus", selectedOptions)
                      }
                      onBlur={handleBlur}
                ...
                <div className="col-span-1 mr-4">
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      isMulti
                      onChange={(selectedOptions) =>
                        setFieldValue("course", selectedOptions)
                      }
                      onBlur={handleBlur}
                      value={values.course}
                    />
                  ...
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      options={leadScource?.map((element, index) => {
                        return {
                          value: element?.LeadSource,
                          label: element?.LeadSource,
                        };
                      })}
                      isMulti
                      onChange={(selectedOptions) =>
                        setFieldValue("leadscource", selectedOptions)
                      }
                      onBlur={handleBlur}
                  ...
                  <div className="mb-4">
                    ...
                    <Field
                      ...
                      onChange={(e) => {
                        ...
                        const tempObj = emailTempObj.find(
                          (element) => element.TemplateID == selectedTemplateId
                        );
                        console.log(tempObj);
                        setFieldValue(
                          "templateBody",
                        ...
                        setFieldValue(
                          "templateSubject",
                          tempObj ? tempObj.TemplateMessage : ""
                        );
                      }}
                    >
              ...
              <div>
                ...
                <div className="col-span-1 border-2 border-solid p-4 rounded border-gray-500">
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="text-white rounded py-2 px-4 absolute right-[.1rem] top-[0.30rem]"
                    >
                      Add
                    </button>
                  </div>
                ...
                <Field
                  ...
                  onChange={(e) => {
                    setFieldValue("templateSubject", e.target.value);
                  }}
                />
                <ErrorMessage
                  name="templateSubject"
              ...
              <div className="mb-4">
                ...
                <Field
                  ...
                  className="mt-1 p-2 w-full rounded border"
                  onChange={(e) => {
                    setFieldValue("templateBody", e.target.value);
                  }}
                />
                <ErrorMessage
                  name="templateBody"
